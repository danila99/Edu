...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)   
    CD = matutil.mat2coldict(M)
    return sum(v[i]*CD[i] for i in CD.keys()) 

## Problem 12
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    CD = matutil.mat2rowdict(M)
    return sum(v[i]*CD[i] for i in CD.keys()) 

## Problem 13
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return Vec(M.D[0], {k:(v1*v) for k,v1 in matutil.mat2rowdict(M).items()})

## Problem 14
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return Vec(M.D[1], {k:(v1*v) for k,v1 in matutil.mat2coldict(M).items()})  

## Problem 15
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    CD = matutil.mat2coldict(B)
    return matutil.coldict2mat({i:A*CD[i] for i in CD.keys()})

## Problem 16
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    CD = matutil.mat2rowdict(A)
    return matutil.rowdict2mat({i:CD[i]*B for i in CD.keys()})
