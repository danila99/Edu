...
def vec_sum(veclist, D): 
    ...
    '''
    return Vec(D, {k:sum(v.f[k] for v in [v for v in veclist if v not in vec_select(veclist, k)]) for k in D})
...
def scale_vecs(vecdict):
    ...
    True
    '''
    return [Vec(v.D, {k:(v.f[k] / scale) for k in v.f.keys()}) for scale, v in vecdict.items()]
...
def GF2_span(D, L):
    ...
    True
    '''
    cortesian_coeff = itertools.product([one, 0], repeat=len(L))
    
    result = []
    for coeff in cortesian_coeff:
        temp = []
        for i in range(len(L)):
            temp.append(vec.scalar_mul(L[i], coeff[i]))
