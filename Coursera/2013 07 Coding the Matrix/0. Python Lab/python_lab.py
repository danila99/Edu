...
## Task 16
first_of_tuples_list = [(z,y,z) for x in S for y in S for z in S if z+y+z==0 and x != y != z][0]

## Task 17
L1 = [1,2,2,3] # <-- want len(L1) != len(list(set(L1)))
L2 = [4,2,3,1] # <-- same len(L2) == len(list(set(L2))) but L2 != list(set(L2))
...
## Task 24
D = {'red','white','blue'}
identity_dict = {x:y for x in D for y in D if x==y}
...
listdict2dict = {names[i]:v for (k,v) in d.items() for i in range(len(names)) if i == k}
