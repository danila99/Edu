...
def basis(vlist):
    ...
    '''
    return [x for x in orthogonalize(vlist) if x*x > 1e-14]
...
def subset_basis(vlist):
    ...
    '''
    orthogonal = [(x if x*x > 1e-14 else Vec(x.D, {})) for x in orthogonalize(vlist)]
    return [v for v in vlist if orthogonal[vlist.index(v)] != Vec(v.D, {})]
